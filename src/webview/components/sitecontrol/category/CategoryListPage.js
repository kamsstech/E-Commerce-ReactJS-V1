...
const CategoryListPage = (props) => {
	...
	useEffect(() => {
	    setloadStatus(false);
	    if (categoryListPageResult.statuscode === 1) {
	      if(categoryListPageResult.payload.data.length > 0)
	      {
	        setarrayJson(categoryListPageResult.payload?.data);
	        setTotalList(pageOffset + categoryListPageResult.payload.data.length)
	        setLimitTotal(categoryListPageResult.payload?.total)
	        var totalcount=categoryListPageResult.payload?.total / filterBy
	        if (Number.isInteger(totalcount)) {
				var totalc=totalcount
			}
			else
	    ...
	    if(categoryListPageResult.statuscode === 3) {
	      setarrayJson([])
	      setloadStatus(false);
	      setNotFound(true);
	    }
								...
								<Table className="paymentsellerList">
									...
									<TableBody>
									{
										...
										<TableRow>
											...
											<TableCell className="tBody">{item1.c_category_name}</TableCell>
											{/*<TableCell className="tBody">{item1.n_category_type}</TableCell>*/}
