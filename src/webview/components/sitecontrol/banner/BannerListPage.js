...
const BannerListPage = (props) => {
	...
	useEffect(() => {
	    setloadStatus(false);
	    if (bannerListPageResult.statuscode === 1) {
	      if(bannerListPageResult.payload.data.length > 0)
	      {
	        setarrayJson(bannerListPageResult.payload?.data);
	        setTotalList(pageOffset + bannerListPageResult.payload.data.length)
	        setLimitTotal(bannerListPageResult.payload?.total)
	        var totalcount=bannerListPageResult.payload?.total / filterBy
	        if (Number.isInteger(totalcount)) {
				var totalc=totalcount
			}
			else
	    ...
	    if(bannerListPageResult.statuscode === 3) {
	      setarrayJson([])
	      setloadStatus(false);
	      setNotFound(true);
	    }
		...
		<>
			<div>
				<div className="profile-title-sec ml-16">
					<h4 className="profile-title">Banner List</h4>
				</div>
				<div>
