...
const VariationsListPage = (props) => {
	const {
		variationsListPageResult,
		VariationsListPageAction,
	} = props;
	...
	useEffect(() => {
	    setloadStatus(false);
	    if (variationsListPageResult.statuscode === 1) {
	      if(variationsListPageResult.payload.data.length > 0)
	      {
	        setarrayJson(variationsListPageResult.payload?.data);
	        setTotalList(pageOffset + variationsListPageResult.payload.data.length)
	        setLimitTotal(variationsListPageResult.payload?.total)
	        var totalcount=variationsListPageResult.payload?.total / filterBy
	        if (Number.isInteger(totalcount)) {
				var totalc=totalcount
			}
			else
	    ...
	    if(variationsListPageResult.statuscode === 3) {
	      setarrayJson([])
	      setloadStatus(false);
	      setNotFound(true);
	    }
		...
		<>
			<div>
				<div className="profile-title-sec ml-16">
					<h4 className="profile-title">Variation List</h4>
				</div>
				<div>
