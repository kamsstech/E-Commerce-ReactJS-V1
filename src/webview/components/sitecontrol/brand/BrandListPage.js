...
const BrandListPage = (props) => {
	...
	useEffect(() => {
	    ...
	    if (brandListPageResult.statuscode === 1) {
	      ...
	      {
	        setarrayJson(brandListPageResult.payload?.data);
	        setTotalList(pageOffset + brandListPageResult.payload.data.length)
	        setLimitTotal(brandListPageResult.payload?.total)
	        var totalcount=brandListPageResult.payload?.total / filterBy
	        if (Number.isInteger(totalcount)) {
				var totalc=totalcount
			}
			else
	    ...
	    if(brandListPageResult.statuscode === 3) {
	      setarrayJson([])
	      setloadStatus(false);
	      setNotFound(true);
	    }
		...
		<>
			<div>
				<div className="profile-title-sec ml-16">
					<h4 className="profile-title">Brand List</h4>
				</div>
				<div>
