...
function GrowTransition(props) {
    console.log('props in freq',props)
    return <Grow {...props} />;
}
const FreqListView = (props ) => {
const {freqorderResponse} =props
    ...
    const handleClickWishList = (Transition, name) => () => {
        setTimeout(() =>{
    ...
    const handleClickWishlistRemoved = (Transition, name) => () => {
        setTimeout(() =>{
          setState({
            open: true,
            Transition,
    ...
    return (
        <div>
            <div className="list-view-sec-1">
                {
                    Array.isArray(freqorderResponse.payload) && freqorderResponse.payload.length > 0 && (freqorderResponse.payload).map((item, index) => (
                        <div className="list-view-sec-25-1" key={item.c_item_code}>
                            <div className="list-view-tile-1" >
                                ...
                                <Tooltip title="Shortbook" TransitionComponent={Zoom}>
                                            <div className="list-view-tile-shortbook-1" onClick={handleIconColor(index)}>
                                        ...
                                        <div className="list-view-tile-wishList-1" onClick={handleWishListColor( index)}>
                                        {wishListColor[index] === true ?  
                                        <img src={wishlist_colorIcon} alt="wishListImg-1" className="wishList-icon"  onClick={handleClickWishlistRemoved(GrowTransition,  item.c_item_name)}  /> :
                                        <img src={wishlist} alt="wishListImg-1" className="wishList-icon" onClick={handleClickWishList(GrowTransition,  item.c_item_name)} />}
                                        </div>
                                    </Tooltip>
                                    <div>
                                        <img src={commerce_offer} alt="wishListImg-1" className="list-view-tile-fav-1" />
                                    ...
                                    <div className="list-view-right-side-sec freq-list">
                                        <Button variant="contained" color="primary" className="list-view-addtocart-1" onClick={handleOpen}>
                                            Add To Cart
                                            </Button>
                                    </div>
                                </div>
