...
const ItemNAInfo = (props) => {
	...
	const { errorMsgItemDetails, GetPdpNAPageItems, pdpNAPageItemsResult } = props;
	...
	let data = pdpNAPageItemsResult.payload;
				...
				data.map((item,index)=>(
					...
					<div className="pdp-flexinfo-sec">
						...
						<div className="relative">
							...
							{/*<h4 className="share-link" onClick={()=>setOpenComingSoon(true)}>
					...
					<p className="pdp-item-mfc">Mfg by: {item.c_mfg_name}</p>
					<p className="pdp-item-mfc">Contains: {item.c_content_name}</p>
					<p className="pdp-item-mfc">HSN Code: {item.c_hsn_code}</p>
					...
					>
						...
						{item.j_related_items && item.j_related_items.map((item, index) => (
							...
							<Tab label={item.c_pack_name} key={index} onClick={() => GetPdpNAPageItems(item.c_item_code)} />
						))}
